...
var loadConfig = () => {
	try {
		if ((0, fs.existsSync)(configPath)) {
			...
			const data = (0, fs.readFileSync)(configPath, "utf8");
			const loaded = {
				...defaultConfig,
				...JSON.parse(data)
			};
			...
			return loaded;
		} else console.log("[Config] No config file found, using defaults");
	} catch (error) {
...
var saveConfig = (config) => {
	try {
		...
		(0, fs.writeFileSync)(configPath, JSON.stringify(config, null, 2), "utf8");
...
var artNetPath = electron.app.isPackaged ? (0, path.join)(process.resourcesPath, "artNetTc.js") : (0, path.join)(__dirname, "..", "..", "src", "electron", "artNetTc.js");
var artNet = (0, child_process.fork)(artNetPath, { stdio: [
	"pipe",
	"pipe",
	"pipe",
	"ipc"
...
var createWindow = () => {
	win = new electron.BrowserWindow({
		...
		show: false,
		title: "ArtTimecode Gen v" + electron.app.getVersion(),
		transparent: true,
		frame: false,
		resizable: false,
		hasShadow: false,
		icon: (0, path.join)(__dirname, "icon.ico"),
		webPreferences: {
	...
	console.log("[DEBUG] app.isPackaged:", electron.app.isPackaged);
	...
	if (!electron.app.isPackaged) win.webContents.openDevTools({ mode: "detach" });
};
electron.app.on("ready", () => {
	configPath = (0, path.join)(electron.app.getPath("userData"), "config.json");
	console.log("[Config] Config path:", configPath);
	currentConfig = loadConfig();
	console.log("[Config] Initializing artNet with saved config");
	artNet.send({
		cmd: "rate",
		rate: currentConfig.frameRate
	});
	...
	if (currentConfig.output && currentConfig.consoleAddress) {
		artNet.send({
			cmd: "consoleAddress",
			address: currentConfig.consoleAddress
		});
	...
	electron.protocol.registerFileProtocol("atom", (request, callback) => {
		...
		callback({ path: path.default.normalize(`${__dirname}/${url}`) });
	});
	artNet.on("message", (msg) => {
		switch (msg.cmd) {
			...
			case "speed":
				...
				break;
			case "output": myEmitter.emit("output", msg.output);
		}
	});
	artNet.on("error", (err) => {
		console.log(err);
	...
	electron.ipcMain.on("reactIsReady", () => {
		...
		if (electron.app.isPackaged) {
			...
			import_main.autoUpdater.on("error", (err) => win?.webContents.send("updater", err));
			import_main.autoUpdater.on("checking-for-update", () => win?.webContents.send("updater", "checking-for-update"));
			import_main.autoUpdater.on("update-available", (info) => win?.webContents.send("updater", "update-available", info));
			import_main.autoUpdater.on("update-not-available", (info) => win?.webContents.send("updater", "update-not-available", info));
			import_main.autoUpdater.on("download-progress", (info) => win?.webContents.send("updater", "download-progress", info));
			import_main.autoUpdater.on("update-downloaded", (info) => win?.webContents.send("updater", "update-downloaded", info));
			electron.ipcMain.on("installUpdate", () => import_main.autoUpdater.quitAndInstall());
			setTimeout(() => import_main.autoUpdater.checkForUpdates(), 3e3);
			setInterval(() => import_main.autoUpdater.checkForUpdates(), 36e5);
		}
	});
	...
	electron.ipcMain.handle("consoleAddress", async (_e, address) => {
	...
	electron.ipcMain.handle("frameRate", async (_e, rate) => {
	...
	electron.ipcMain.handle("state", async (_e, state) => {
		return await setState(state);
	});
	electron.ipcMain.handle("speed", async (_e, speed) => {
	...
	electron.ipcMain.handle("startTime", async (_e, startTime) => {
	...
	electron.ipcMain.on("time", (_e, time) => {
		artNet.send({
			cmd: "time",
			time
		});
	...
	electron.ipcMain.on("min", () => win?.minimize());
	electron.ipcMain.on("setClickThrough", (_e, enabled) => {
