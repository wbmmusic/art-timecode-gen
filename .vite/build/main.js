...
const myEmitter = new require$$0$2.EventEmitter();
...
const loadConfig = () => {
  try {
    if (require$$1.existsSync(configPath)) {
      console.log("[Config] Loading config from:", configPath);
      const data = require$$1.readFileSync(configPath, "utf8");
    ...
    console.error("[Config] Failed to load config:", error2);
  }
...
const saveConfig = (config) => {
  try {
    ...
    require$$1.writeFileSync(configPath, JSON.stringify(config, null, 2), "utf8");
    currentConfig = { ...config };
    console.log("[Config] Config saved successfully to:", configPath);
  } catch (error2) {
    console.error("[Config] Failed to save config:", error2);
  }
};
...
if (!gotTheLock) {
  ...
  require$$1$4.app.on("second-instance", () => {
    if (win) {
...
const artNetPath = require$$1$4.app.isPackaged ? require$$1$1.join(process.resourcesPath, "artNetTc.js") : require$$1$1.join(__dirname, "..", "..", "src", "electron", "artNetTc.js");
const artNet = require$$1$5.fork(artNetPath, { stdio: ["pipe", "pipe", "pipe", "ipc"] });
...
const createWindow = () => {
  win = new require$$1$4.BrowserWindow({
    ...
    show: false,
    title: "ArtTimecode Gen v" + require$$1$4.app.getVersion(),
    transparent: true,
    frame: false,
    resizable: false,
    hasShadow: false,
  ...
  console.log("[DEBUG] MAIN_WINDOW_VITE_DEV_SERVER_URL:", "http://localhost:5173");
  console.log("[DEBUG] MAIN_WINDOW_VITE_NAME:", "main_window");
  console.log("[DEBUG] __dirname:", __dirname);
  console.log("[DEBUG] app.isPackaged:", require$$1$4.app.isPackaged);
  {
    console.log("[DEBUG] Loading from dev server:", "http://localhost:5173");
    win.loadURL("http://localhost:5173");
  }
  win.webContents.on("did-fail-load", (event, errorCode, errorDescription, validatedURL) => {
    console.error("[DEBUG] Failed to load page:", errorCode, errorDescription, validatedURL);
  });
  win.on("closed", () => require$$1$4.app.quit());
  win.on("ready-to-show", () => win?.show());
  if (!require$$1$4.app.isPackaged) {
    win.webContents.openDevTools({ mode: "detach" });
  }
};
require$$1$4.app.on("ready", () => {
  configPath = require$$1$1.join(require$$1$4.app.getPath("userData"), "config.json");
  ...
  require$$1$4.protocol.registerFileProtocol("atom", (request, callback) => {
    const url = request.url.substr(6);
    callback({ path: require$$1$1.normalize(`${__dirname}/${url}`) });
  });
  ...
  require$$1$4.ipcMain.on("reactIsReady", () => {
    ...
    if (require$$1$4.app.isPackaged) {
      ...
      mainExports.autoUpdater.on("error", (err) => win?.webContents.send("updater", err));
      mainExports.autoUpdater.on(
        "checking-for-update",
        () => win?.webContents.send("updater", "checking-for-update")
      );
      mainExports.autoUpdater.on(
        "update-available",
        (info) => win?.webContents.send("updater", "update-available", info)
      );
      mainExports.autoUpdater.on(
        "update-not-available",
        (info) => win?.webContents.send("updater", "update-not-available", info)
      );
      mainExports.autoUpdater.on(
        "download-progress",
        (info) => win?.webContents.send("updater", "download-progress", info)
      );
      mainExports.autoUpdater.on(
        "update-downloaded",
        (info) => win?.webContents.send("updater", "update-downloaded", info)
      );
      require$$1$4.ipcMain.on("installUpdate", () => mainExports.autoUpdater.quitAndInstall());
      setTimeout(() => mainExports.autoUpdater.checkForUpdates(), 3e3);
      setInterval(() => mainExports.autoUpdater.checkForUpdates(), 1e3 * 60 * 60);
    }
  });
  ...
  require$$1$4.ipcMain.handle("consoleAddress", async (_e, address) => {
  ...
  require$$1$4.ipcMain.handle("frameRate", async (_e, rate) => {
  ...
  require$$1$4.ipcMain.handle("state", async (_e, state) => {
    return await setState(state);
  });
  require$$1$4.ipcMain.handle("speed", async (_e, speed) => {
  ...
  require$$1$4.ipcMain.handle("startTime", async (_e, startTime) => {
  ...
  require$$1$4.ipcMain.on("time", (_e, time) => {
    artNet.send({ cmd: "time", time });
  });
  require$$1$4.ipcMain.on("close", () => require$$1$4.app.quit());
  require$$1$4.ipcMain.on("min", () => win?.minimize());
  require$$1$4.ipcMain.on("setClickThrough", (_e, enabled) => {
...
require$$1$4.app.on("will-quit", () => artNet.kill("SIGKILL"));
